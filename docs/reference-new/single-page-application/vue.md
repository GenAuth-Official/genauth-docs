...
首先你要在 [GenAuth Console](https://console.genauth.ai) 中获取你的 `应用 ID`。如果你不知道如何获取 `应用 ID`，可以[参考文档](../../guides/faqs/get-app-id-and-secret.md)
...
Guard 组件本身基于 [GenAuth JavaScript SDK](../web/sdk-for-node/) 进行封装，当你需要进行一些更高级的操作（如管理用户自定义数据、修改用户资料、退出登录）时：
...
**私有化部署**场景需要指定你私有化的 GenAuth 服务的 Host（**包含协议但不含 Path**），如果你不清楚可以联系 GenAuth IDaaS 服务管理员。
