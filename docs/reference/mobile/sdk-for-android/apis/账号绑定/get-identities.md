...
`open_id`、`union_id` 和 `user_id`。在 GenAuth 中，我们把这样的一条 `open_id` 或者 `unionid_` 叫做一条 `Identity`， 所以用户在一个身份源会有多条 `Identity` 记录。
...
可以看到他们的 `extIdpId` 是一样的，这个是你在 GenAuth 中创建的**身份源 ID**；`provider` 都是 `wechat`；
